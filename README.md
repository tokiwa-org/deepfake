...
> ⚠️ **品質について**: 本プロジェクトの軽量版モデル（**670万パラメータ**）では、顔交換結果は**ぼやけて全く判別できません**。これはアルゴリズムの仕組みを理解するための教育用実装であり、実用的な品質を得るには商用フレームワーク（DeepFaceLab、SimSwap等）が必要です。
...
> ⚠️ **重要**: Deepfakeは**特定の2人の人物**の顔を交換する技術です。
> - `person_a/` には**人物Aだけ**の顔画像を配置
> - `person_b/` には**人物Bだけ**の顔画像を配置
>
> FFHQのような多様な人物が含まれるデータセットでは、モデルは「平均的な顔」しか学習できず、結果がぼやけます。
